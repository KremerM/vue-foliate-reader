import { f as z, p as V, t as X, a as W } from "./index-CltvV-nZ.js";
const b = {
  ...
  26: "doi",
  34: "issn"
}, B = (i) => i.toLowerCase().replace(/[-:](.)/g, (t, e) => e.toUpperCase()), K = (i) => i ? i.replace(/[\t\n\f\r ]+/g, " ").replace(/^[\t\n\f\r ]+/, "").replace(/[\t\n\f\r ]+$/, "") : "", Q = (i, t, e) => e ? (s) => s.getAttribute(i)?.split(/\s/)?.includes(t) : typeof t == "function" ? (s) => t(s.getAttribute(i)) : (s) => s.getAttribute(i) === t, U = (...i) => (t) => t ? Object.fromEntries(i.map((e) => [B(e), t.getAttribute(e)])) : null, A = (i) => K(i?.textContent), R = (i, t) => {
...
}, Z = (i) => i.slice(0, i.lastIndexOf("/") + 1), C = async (i, t, e) => {
  const s = [];
  ...
  const { $: t } = R(i, b.OPF), e = t(i.documentElement, "metadata"), s = Object.groupBy(e.children, (a) => a.namespaceURI === b.DC ? "dc" : a.namespaceURI === b.OPF && a.localName === "meta" ? a.hasAttribute("name") ? "legacyMeta" : "meta" : ""), r = e.getAttribute("xml:lang") ?? i.documentElement.getAttribute("xml:lang") ?? "und", o = et(i), n = (a) => {
  ...
  }, p = Object.fromEntries(Object.entries(Object.groupBy(s.dc, (a) => a.localName)).map(([a, f]) => [a, f.map(n)])), d = c() ?? {}, h = Object.fromEntries(s.legacyMeta?.map((a) => [a.getAttribute("name"), a.getAttribute("content")]) ?? []), u = (a) => a?.[0]?.value, g = (a, f) => u(a?.props?.[f]), m = (a) => {
    if (!a) return null;
    ...
    position: u(a.props?.["group-position"])
  }), S = (a) => {
    const { value: f } = a;
  ...
  ), x = p.title?.find((a) => g(a, "title-type") === "main") ?? p.title?.[0], L = {
    identifier: _(i),
    title: m(x),
    sortAs: m(x?.props?.["file-as"]?.[0]) ?? x?.attrs?.["file-as"] ?? h?.["calibre:title_sort"],
    subtitle: p.title?.find((a) => g(a, "title-type") === "subtitle")?.value,
    language: p.language?.map((a) => a.value),
    description: u(p.description),
    publisher: y(p.publisher?.[0]),
    published: p.date?.find((a) => a.attrs.event === "publication")?.value ?? u(p.date),
    modified: u(d[T.dcterms + "modified"]) ?? p.date?.find((a) => a.attrs.event === "modification")?.value,
    subject: p.subject?.map(y),
    belongsTo: {
    ...
    altIdentifier: p.identifier?.map(S),
    source: p.source?.map(S),
  ...
  for (const [a, f] of [].concat(
    p.creator?.map(y)?.map(P("author")) ?? [],
    p.contributor?.map(y)?.map(P("contributor")) ?? []
  ))
    for (const w of a)
      L[w] ? L[w].push(f) : L[w] = [f];
  O(L), L.altIdentifier === L.identifier && delete L.altIdentifier;
  const H = {}, N = {};
  for (const [a, f] of Object.entries(d))
    a.startsWith(T.rendition) ? H[B(a.replace(T.rendition, ""))] = u(f) : a.startsWith(T.media) && (N[B(a.replace(T.media, ""))] = u(f));
  return N.duration && (N.duration = k(N.duration)), { metadata: L, rendition: H, media: N };
}, rt = (i, t = (e) => e) => {
  const { $: e, $$: s, $$$: r } = R(i, b.XHTML), o = (m) => m ? decodeURI(t(m)) : null, n = (m) => (y) => {
    const E = e(y, "a") ?? e(y, "span"), S = e(y, "ol"), $ = o(E?.getAttribute("href")), L = { label: A(E) || E?.getAttribute("title"), href: $, subitems: l(S) };
    return m && (L.type = E?.getAttributeNS(b.EPUB, "type")?.split(/\s/)), L;
  }, l = (m, y) => m ? s(m, "li").map(n(y)) : null, c = (m, y) => l(e(m, "ol"), y), p = r(i, "nav");
  let d = null, h = null, u = null, g = [];
  for (const m of p) {
    const y = m.getAttributeNS(b.EPUB, "type")?.split(/\s/) ?? [];
    y.includes("toc") ? d ??= c(m) : y.includes("page-list") ? h ??= c(m) : y.includes("landmarks") ? u ??= c(m, !0) : g.push({
      label: A(m.firstElementChild),
      type: y,
      list: c(m)
    });
  }
  return { toc: d, pageList: h, landmarks: u, others: g };
}, it = (i, t = (e) => e) => {
  const { $: e, $$: s } = R(i, b.NCX), r = (c) => c ? decodeURI(t(c)) : null, o = (c) => {
    const p = e(c, "navLabel"), d = e(c, "content"), h = A(p), u = r(d.getAttribute("src"));
    if (c.localName === "navPoint") {
      const g = s(c, "navPoint");
      return { label: h, href: u, subitems: g.length ? g.map(o) : null };
    }
    return { label: h, href: u };
  }, n = (c, p) => s(c, p).map(o), l = (c, p) => {
...
class nt extends EventTarget {
  ...
  async #g(t) {
    if (this.#s === t) return;
    const e = await this.loadXML(t.href), s = (n) => n ? M(n, t.href) : null, { $: r, $$$: o } = R(e, b.SMIL);
    this.#n = -1, this.#i = -1, this.#e = o(e, "par").reduce((n, l) => {
      const c = s(r(l, "text")?.getAttribute("src")), p = r(l, "audio");
      if (!c || !p) return n;
      const d = s(p.getAttribute("src")), h = k(p.getAttribute("clipBegin")), u = k(p.getAttribute("clipEnd")), g = n.at(-1);
      return g?.src === d ? g.items.push({ text: c, begin: h, end: u }) : n.push({ src: d, items: [{ text: c, begin: h, end: u }] }), n;
    }, []), this.#s = t;
  }
  get #h() {
  ...
  async #c(t, e) {
    ...
    this.#r = o, o.volume = this.#d, o.playbackRate = this.#f, o.addEventListener("timeupdate", () => {
      ...
      if (n > this.#a?.end && (this.#u(), this.#i === l.length - 1)) {
        this.#c(this.#n + 1, 0).catch((p) => this.#o(p));
        return;
      }
      const c = this.#i;
    ...
    for (let n = 0; n < this.#e.length; n++) {
      ...
      for (let c = 0; c < l.length; c++)
        if (l[c].text.split("#")[0] === r && e(l[c], c, l))
          return this.#c(n, c).catch((p) => this.#o(p));
    }
  }
  pause() {
...
class pt {
  constructor({ opf: t, resolveHref: e }) {
    this.opf = t;
    const { $: s, $$: r, $$$: o } = R(t, b.OPF), n = s(t.documentElement, "manifest"), l = s(t.documentElement, "spine"), c = r(l, "itemref");
    ...
    p && (this.guide = r(p, "reference").map(U("type", "title", "href")).map(({ type: d, title: h, href: u }) => ({
      label: h,
      type: d.split(/\s/),
      href: e(u)
...
class ut {
  ...
  async createURL(t, e, s, r) {
    ...
    const l = await n.detail.data, c = await n.detail.type, p = URL.createObjectURL(new Blob([l], { type: c }));
  ...
  async loadItem(t, e = []) {
    ...
    const p = e.at(-1);
    if (this.#e.has(s)) return this.ref(s, p);
    if ((o || [I.XHTML, I.HTML, I.CSS, I.SVG].includes(r)) && e.every((u) => u !== s)) return this.loadReplaced(t, e);
    const h = Promise.resolve().then(() => this.loadBlob(s));
    return this.createURL(s, h, r, p);
  }
  ...
  async loadReplaced(t, e = []) {
    ...
    if ([I.XHTML, I.HTML, I.SVG].includes(r)) {
      ...
      const p = async (h, u) => h.setAttribute(
        u,
        await this.loadHref(h.getAttribute(u), s, e)
      );
      for (const h of c.querySelectorAll("link[href]")) await p(h, "href");
      for (const h of c.querySelectorAll("[src]")) await p(h, "src");
      for (const h of c.querySelectorAll("[poster]")) await p(h, "poster");
      for (const h of c.querySelectorAll("object[data]")) await p(h, "data");
  ...
  replaceString(t, e, s = []) {
    const r = /* @__PURE__ */ new Map(), o = this.assets.map((l) => {
      if (l.href === e) return;
      const c = Y(Z(e), l.href), p = encodeURI(c), d = "/" + l.href, h = encodeURI(d), u = /* @__PURE__ */ new Set([c, p, d, h]);
      for (const g of u) r.set(g, l);
      return Array.from(u);
    }).flat().filter((l) => l);
    if (!o.length) return t;
    const n = new RegExp(o.map(tt).join("|"), "g");
    return C(t, n, async (l) => this.loadItem(
      r.get(l.replace(/^\//, "")),
      s.concat(e)
    ));
  }
...
const dt = (i, t) => i.getElementById(t) ?? i.querySelector(`[name="${CSS.escape(t)}"]`), ft = (i) => {
  ...
  fixedLayout: A(i.querySelector('option[name="fixed-layout"]')),
  openToSpread: A(i.querySelector('option[name="open-to-spread"]'))
} : null;
class yt {
...
${s.querySelector("parsererror").innerText}`);
  ...
  async init() {
    ...
    }), this.transformTarget = this.#e.eventTarget, this.sections = this.resources.spine.map((u, g) => {
      const { idref: m, linear: y, properties: E = [] } = u, S = this.resources.getItemByID(m);
      return S ? {
        id: S.href,
    ...
    if (n) try {
      const u = (m) => M(m, n), g = rt(await this.#t(n), u);
      this.toc = g.toc, this.pageList = g.pageList, this.landmarks = g.landmarks;
    } catch (u) {
      console.warn(u);
    }
    if (!this.toc && l) try {
      const u = (m) => M(m, l), g = it(await this.#t(l), u);
      this.toc = g.toc, this.pageList = g.pageList;
    } catch (u) {
      console.warn(u);
    }
    this.landmarks ??= this.resources.guide;
    const { metadata: c, rendition: p, media: d } = st(r);
    this.metadata = c, this.rendition = p, this.media = d, this.dir = this.resources.pageProgressionDirection;
    const h = mt(
      await this.#t("META-INF/com.apple.ibooks.display-options.xml") ?? await this.#t("META-INF/com.kobobooks.display-options.xml")
    );
    return h && (h.fixedLayout === "true" && (this.rendition.layout ??= "pre-paginated"), h.openToSpread === "false" && (this.sections.find((u) => u.linear !== "no").pageSpread ??= this.dir === "rtl" ? "left" : "right")), this;
  }
